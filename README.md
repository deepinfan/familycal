...
3. 添加 Postgres 数据库（Storage → Create Database → Postgres）
4. 添加 Blob 存储（Storage → Create Store → Blob）
5. 配置环境变量：
   - `JWT_SECRET`（运行 `bash scripts/generate-keys.sh` 生成）
   - `SYSTEM_CONFIG_AES_KEY`（运行 `bash scripts/generate-keys.sh` 生成）
   - `APP_URL`（使用 Vercel 提供的域名）
6. 部署完成后，数据库会自动迁移
7. 访问部署 URL，使用 admin/admin123 登录

详细部署指南请参考 [DEPLOYMENT.md](./DEPLOYMENT.md)

#### VPS 部署

参考 [DEPLOYMENT.md](./DEPLOYMENT.md) 中的 VPS 部署章节。
